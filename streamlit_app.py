...
PARK_FACTORS = {
    ...
    "Los Angeles Dodgers": 0.92,   # ← V4.32: dropped from 0.95, elite pitching staff
    "San Francisco Giants": 0.94, "San Diego Padres": 0.92,
}
...
PITCHER_ERA_FALLBACK = {
    ...
    "Casey Mize": 4.30,
    "Keider Montero": 4.50,
    "Jack Kochanowicz": 4.40,
    "Brandon Williamson": 4.20,
    "Jacob Lopez": 4.60,
    "Erick Fedde": 4.40,
    "Michael Wacha": 4.10,
    "Foster Griffin": 4.70,
    ...
    "Clay Holmes": 3.80,
    "Kodai Senga": 3.50,
    "Braxton Ashcraft": 4.30,
    "Miles Mikolas": 6.50,
    "Aaron Nola": 4.20,
    "Merrill Kelly": 4.10,
    "Patrick Detmers": 4.60,
    "Ryan Weathers": 2.81,
    "Spencer Springs": 3.20,
    "Bryce Elder": 2.50,
    "Will Warren": 3.07,
    "Cade Cavalli": 2.51,
    "Jake Abel": 6.08,
...
def signal_boxes(model_total, line, price_cents, game_id, prefix, away, home,
    ...
    # If Odds API is down AND line is the default, skip all signals.
    # This was the root cause of 7 of 9 losses — false edge vs default lines.
    _is_default_fg = (market_type == "full" and line == DEFAULT_FG_LINE and not odds_lines)
    _is_default_f5 = (market_type == "f5" and line == DEFAULT_F5_LINE and not odds_lines)
    if _is_default_fg or _is_default_f5:
        st.warning(
            f"⚠️ **Default line ({line}) — Odds API unavailable.** "
            f"No signals until real Vegas line loads. Check back once Odds API resets."
        )
        return 0.0, 0.0
        ...
        if not schedule:
            ...
            for g in schedule:
                try:
                    ...
                    # Default line protection in summary table too
                    ...
                    elif _f5_lean == "EVEN" or _f5_edge < EDGE_THRESHOLD:
                        _f5_signal = "⚪ NO EDGE"
                    ...
                    elif _fg_lean == "EVEN" or _fg_edge < EDGE_THRESHOLD:
                        _fg_signal = "⚪ NO EDGE"
            ...
            if rows:
                ...
                if not odds_lines:
                    st.error("⛔ Odds API unavailable — default lines in use. No bets will fire until real Vegas lines load.")

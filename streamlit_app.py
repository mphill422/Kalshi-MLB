...
PITCHER_ERA_FALLBACK = {
    ...
    "Jose Suarez": 4.70,
    "Aaron Civale": 4.50,
    "Bryce Miller": 4.20,           # 🟡 bounce back expected
    "Tylor Megill": 4.30,
    "Jose Quintana": 4.50,
    "Charlie Morton": 4.40,
    "Dane Dunning": 4.60,
    ...
    "Yu Darvish": 3.80,
    "Mitch Keller": 3.91,           # 🟢 2 ER in 18 IP per today's slate notes
    "Jameson Taillon": 3.68,
    "Shane McClanahan": 3.86,
    "Sonny Gray": 3.80,             # 🟢 shut out Brewers last start
    "Spencer Schwellenbach": 3.00,  # 🟢 3.00 ERA/1.21 WHIP before 2025 injury
    "Cade Horton": 2.67,            # 🟢 Cubs rookie — 2.67 ERA/3.58 FIP in 2025
    # ── New / updated 2026 pitchers ───────────────────────────────────────────
    "Yusei Kikuchi": 6.76,          # 🟢 11 runs in 14.2 IP
    "Ryne Nelson": 4.20,            # 🟢 xERA 5.61 — regression risk
    "Javier Assad": 3.20,           # 🟢 great first start
    ...
    "Kodai Senga": 3.50,            # 🟡 Mets — returning, velo back up
    "Braxton Ashcraft": 4.30,       # 🟡 Pirates
    "Miles Mikolas": 6.50,          # 🟢 12.41 ERA early — heavily regressed estimate
    "Aaron Nola": 4.20,             # 🟡 Phillies — finding consistency
    "Merrill Kelly": 4.10,          # 🟡 Diamondbacks — season debut today
    "Patrick Detmers": 4.60,        # 🟢 4.60 ERA, xERA 2.50 — due for regression down
    "Ryan Weathers": 2.81,          # 🟢 2.81 ERA, xERA 5.06 — regression risk up
    "Spencer Springs": 3.20,        # 🟢 0 HR allowed through 3 starts, 18.1 IP
    "Bryce Elder": 2.50,            # 🟢 2 starts 0 ER — regressed to 2.50
    "Will Warren": 3.07,            # 🟢 Yankees confirmed
    "Cade Cavalli": 2.51,           # 🟢 xERA 4.22 — regression coming
    "Aaron Civale": 4.50,           # 🟡
    "Jake Abel": 6.08,              # 🟢 6.08 ERA, xERA 7.08 — struggling
    "Colin Rea": 3.60,              # 🟢 solid first start 2026
}
...
def fetch_recent_team_rpg(team_name, n_games=10):
    ...
    try:
        ...
        sched_resp = _req.get(
            ...
            f"&startDate={start}&endDate={today}&sportId=1&gameType=R", timeout=8)
    ...
    if supabase_connected:
        try:
            ...
            if data.data:
                ...
                view_mode = st.radio(
                    "View", ["All Model Bets", "Real Kalshi Bets Only"], horizontal=True)

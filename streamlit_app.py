...
st.caption(f"Version 1.1 — {datetime.today().strftime('%B %d, %Y')}")
...
try:
    ...
    if not schedule:
        ...
        for game in schedule:
            ...
            st.markdown(f"**{away} @ {home}** — {time_str}")
            st.caption(f"Away SP: {away_pitcher} | Home SP: {home_pitcher}")
            st.divider()
...
Also add `pytz` to your `requirements.txt` file. Open that file, edit it to:
```
streamlit
mlb-statsapi
pandas
